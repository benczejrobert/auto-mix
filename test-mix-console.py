...
exec(open("./params-preproc.txt", 'r').read())
exec(open("./params-features.txt", 'r').read())
...
if proc_end_to_end:
    ...
    shutil.copyfile("params-preproc.txt", os.path.join(out_signals_root_folder, f"params-preproc-{today}.txt"))
...
if create_training_features:
    today = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S-%f")
    shutil.copyfile("params-features.txt", os.path.join(extracted_features_folder, f"params-features-{today}.txt"))
    feats_extractor = FeatureExtractor(feature_list, feature_dict, variance_type, raw_features, keep_feature_dims)
